...
# This file was *autogenerated* from the file ComputePairs.sage
from sage.all_cmdline import *   # import sage library

_sage_const_5000000 = Integer(5000000); _sage_const_1 = Integer(1); _sage_const_400000 = Integer(400000); _sage_const_6 = Integer(6); _sage_const_0 = Integer(0); _sage_const_20 = Integer(20); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5); _sage_const_7 = Integer(7); _sage_const_13 = Integer(13); _sage_const_50 = Integer(50); _sage_const_1000 = Integer(1000); _sage_const_4 = Integer(4); _sage_const_11 = Integer(11); _sage_const_500000 = Integer(500000); _sage_const_10 = Integer(10); _sage_const_1024 = Integer(1024)#%autoindent off
...
# exists).
def test_cong(p, E1, E2, mumax=_sage_const_5000000 , semisimp=True, verbose=False, twist=True):
    ...
    if twist:
        ...
        if d!=_sage_const_1 :
            ...
            if N2>_sage_const_400000 : # we have made E2 worse, so untwist
                if verbose:
    ...
    S = S1+S2
    M = N1.lcm(N2) * prod(S,_sage_const_1 )
    mu = M * prod([(ell+_sage_const_1 )/ell for ell in M.support()])
    mu6 = ZZ(int(mu/_sage_const_6 )) # rounds down
    ...
    for ell in prime_range(mu6):
        ...
        a2 = E2.ap(ell)
        if ell.divides(N1N2):
            if N1N2.valuation(ell)==_sage_const_1  and (a1*a2-(ell+_sage_const_1 ))%p:
    ...
    # neither does)
    if N1.valuation(p)<=_sage_const_1  and N2.valuation(p)<=_sage_const_1 :
        return True, "ss"
    # Giving up....
...
def make_hash(p, N1, N2, np=_sage_const_20 ):
    plist = [next_prime(_sage_const_400000 )]
    while len(plist)<np:
        plist.append(next_prime(plist[-_sage_const_1 ]))
    hashtab = {}
    nc = _sage_const_0 
    cremona_curves = cremona_optimal_curves(srange(N1,N2+_sage_const_1 ))
...
def test_irred(s, p):
    E = EllipticCurve(s[_sage_const_0 ])
    if p in [_sage_const_2 ,_sage_const_3 ,_sage_const_5 ,_sage_const_7 ,_sage_const_13 ]:
        isog_pol = Fricke_polynomial(p)(x)-x*E.j_invariant()
        return len(isog_pol.roots())==_sage_const_0 
    else:
...
def isogeny_kernel_field(phi, optimize=False):
    ...
    xP=pol.roots(Fx)[_sage_const_0 ][_sage_const_0 ]
    yP=phi.domain().lift_x(xP, extend=True)[_sage_const_1 ]
    Fxy = yP.parent()
    Fxy = Fxy.absolute_field('c')
    if optimize:
        Fxy = Fxy.optimized_representation()[_sage_const_0 ]
    return Fxy
...
def check_if_star_is_zero(E, p, prLim = _sage_const_5 ):
    """
    ...
    if type(E) == type("str"):
        ...
        lab = E.cremona_label()
    badp = set([x[_sage_const_0 ] for x in list(E.conductor().factor())] + [p])
...
def is_star_really_zero(lab, p):
    ...
    return (mod(torFld.degree(), p) != _sage_const_0 )
...
def EC_star_field(E, p, optimize=True):
    ...
    star_pol = factors_of_degree_p[_sage_const_0 ]  # Take the first one if multiple exist
    assert star_pol.is_irreducible()
    F = NumberField(star_pol, 's')
    if optimize:
        F = F.optimized_representation()[_sage_const_0 ]
    return F
...
def pairs_mod_p_with_same_star_field(list, p):
    ...
    """
    pairs = []
    starfield_dict = {EC_star_field(list[_sage_const_0 ], p, optimize=False) : [list[_sage_const_0 ]]}
    for i in range(_sage_const_1 , len(list)):
    ...
    for k, v in starfield_dict.items():
        if len(v) < _sage_const_2 :
            continue
        for i in range(len(v)):
            for j in range(i+_sage_const_1 , len(v)):
                E1 = EllipticCurve(v[i])
                E2 = EllipticCurve(v[j])
                if E1.is_isogenous(E2) and gcd(E1.isogeny_degree(E2), p) == _sage_const_1 :
                    continue
...
def remove_star_zero_cases_singleList(ss, p, prLim = _sage_const_50 ):
    alive = []
    for lab in ss:
        E = EllipticCurve(lab)
...
def remove_star_zero_cases(ss, p, prLim = _sage_const_50 ):
    """
...
def field_isomorphism(K1, K2):
    ...
    return magma.eval('IsIsomorphic(K1, K2);')[_sage_const_0 :_sage_const_4 ] == 'true'
...
def compute_and_separate_lists_by_redtype(p):
    ...
    try:
        ...
        modp_hashtable = make_hash(p,_sage_const_11 ,_sage_const_500000 ,_sage_const_50 )
        modp_hashtable = dict([(k,v) for k,v in modp_hashtable.items() if len(v)>_sage_const_1 ])
...
def irreducible_pairs(isom_sets_irred, p):
    ...
    for s in tqdm(isom_sets_irred):
        E1 = EllipticCurve(s[_sage_const_0 ])
        for r in s[_sage_const_1 :]:
            E2 = EllipticCurve(r)
            res, info = test_cong(p,E1,E2, mumax=_sage_const_10 **_sage_const_7 )
            if not res:
                report(res,info,p,s[_sage_const_0 ],r)
                bad_pairs.append([s[_sage_const_0 ],r])
    print("Found {} bad pairs".format(len(bad_pairs)))
    assert len(bad_pairs)==_sage_const_0 , f"There are bad pairs in mod {p} isomorphism sets"
    ...
    for i,s in enumerate(isom_sets_irred_sat):
        for j1 in range(len(s)):
            for j2 in range(j1+_sage_const_1 , len(s)):
                E1 = EllipticCurve(s[j1])
                E2 = EllipticCurve(s[j2])
                if E1.is_isogenous(E2):
                    continue
                out.write('["{}", "{}"]'.format(s[j1],s[j2]));
                if j2<len(s)-_sage_const_1  or i<len(isom_sets_irred_sat)-_sage_const_1 :
                    out.write(',');
                out.write("\\\n");
    out.write('];\n');
    out.close()
...
def reducible_pairs(isom_sets_red, p):
    ...
    for ss in tqdm(isom_sets_red_sat_nonzero):
        ...
        isogenies = [E.isogenies_prime_degree(p)[_sage_const_0 ] for E in base_curves]
        isog_curves = [phi.codomain() for phi in isogenies]
        fld_chi = isogeny_kernel_field(isogenies[_sage_const_0 ], optimize=True)
        kernel_fld_chi = []
        kernel_fld_chip = []
    ...
    for i,s in enumerate(pairs):
        out.write('["{}", "{}"]'.format(s[_sage_const_0 ],s[_sage_const_1 ]))
        if i<len(pairs)-_sage_const_1 :
            out.write(',')
        out.write("\\\n")
    out.write('];\n')
    out.close()
...
def GbToBytes(n):
    return n * _sage_const_1024  * _sage_const_1024  * _sage_const_1024 

pari.allocatemem(GbToBytes(_sage_const_10 ))
...
# # Mod 7
isom_sets_irred, isom_sets_red = compute_and_separate_lists_by_redtype(_sage_const_7 )
irreducible_pairs(isom_sets_irred, _sage_const_7 )
reducible_pairs(isom_sets_red, _sage_const_7 )
